...
export default class DayWaveRewardInfo {
    ...
    public static installCSV(csv: CSV): void {
        ...
        this._hashDic = new Dictionary<string, DayWaveRewardInfo>();
        for (var id in data) {
            let item = data[id];
            ...
            this._hashDic.add(id, new DayWaveRewardInfo(dic));
        }
    }
    /**
     * 获取数据数量
     ...
     * 通过id获取Info
     */
    public static getInfo(id: any): DayWaveRewardInfo {
        return this._hashDic.getValue(id);
    }

    private static _hashDic: Dictionary<string, DayWaveRewardInfo> = new Dictionary<string, DayWaveRewardInfo>();
    public static server(dic: Dictionary<string, string>[]) {
        this._hashDic = new Dictionary<string, DayWaveRewardInfo>();
        for (var id in dic) {
            let _id = dic[id].getValue("id");
            this._hashDic.add(_id, new DayWaveRewardInfo(dic[id]));
        }
    }
    public static serverSimple(data: any): void {
        ...
        this._hashDic.set(dic.getValue("id"), new DayWaveRewardInfo(dic));
    }
    //构造函数
