...
export default class UI_UpLevel extends fui_UpLevel {
	...
	private refreshHeroLevel(): void {
		...
		if (Game.redData.levelRed) {
			Game.redTip.showRedTip(this.m_upLevel);
		}
		else {
			Game.redTip.hideRedTip(this.m_upLevel);
		}
	...
	private refreshHeroStar(): void {
		...
		if (Game.redData.starRed) {
			Game.redTip.showRedTip(this.m_upStar);
		}
		else {
			Game.redTip.hideRedTip(this.m_upStar);
		}
	...
	private levelUpDown(): void {
		...
		if (Game.playData.upLevelCost() <= Game.playData.curGold) {
			...
			this.showEff = true;
			let str: string = Game.tipTxt.txts("levelupsuccess");
	...
	private starUpClick(): void {
		...
		if (Game.redData.curStarCost() <= Game.playData.curJadeite) {
			if (Game.playData.curStar < Math.floor((Game.playData.curLevel - 1) / 10)) {
				this.showEff = true;
				let str: string = Game.tipTxt.txts("levelbigupsuccess");
				Game.total.toastMsg(str, true, true);
				Game.proto.upStar({});
			}
			else {
